...
const SlideInModal: React.FunctionComponent<SlideInModalProps> = ({ children, isOpen, onClose }) => {
    ...
    return (
        <div className={`slide-in transition-all ${isOpen ? 'duration-0' : 'duration-300'} ${isOpen ? 'w-full' : 'w-0'} fixed h-screen bg-slate-400/60 top-0 right-0 z-30`} onClick={onClose}>
