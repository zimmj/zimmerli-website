...
const Accordion: React.FunctionComponent<AccordionProps> = ({
  ...
  return (
    <div
      className={`group collapse collapse-arrow my-4 bg-neutral ${controllClass} ${
        controllClass === '' ? '' : 'cursor-pointer'
      }`}
    ...
    >
      ...
      <div
        className="collapse-content cursor-auto mt-2"
        onClick={(event) => {
...
export const ControlledAccordion: React.FunctionComponent<
  AccordionProps
> = ({ children, title, accordionId }) => {
  ...
  return (
    <Accordion
      ...
      controllClass={isOpen ? 'collapse-open' : 'collapse-close'}
    >
      {children}
    </Accordion>
  );
