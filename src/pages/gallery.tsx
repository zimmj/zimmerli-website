...
import GalleryPicture, {
  PictureFormat,
} from '../components/Gallery/gallery-picture';
...
const LandScapeGallery: React.FunctionComponent = ({ data }) => {
  ...
  const [picture, setPicture] = React.useState<PictureFormat | null>(
    null,
  );
  ...
  function createCutPictureTag(
    ...
    if (maxSpan == span) {
      return cloudinaryImage
        .resize(
          crop()
            .width(pictureColumnWidth * span + amountOfGap)
            .height(pictureColumnWidth),
        )
        .resize(
          thumbnail()
            .width(columnWidth * span + amountOfGap)
            .height(columnWidth),
        );
    } else {
  ...
  function calculatePictures(): React.ReactNode {
    const pictures: React.ReactNode =
      data.allCloudinaryMedia.edges.map(
        (node: any, index: number) => {
          const media: ICloudinaryMedia = node.node;
          const span = Math.min(
            Math.floor(media.originalWidth / media.originalHeight),
            maxSpan,
          );
  ...
  const pictures: React.ReactNode = useMemo(
    () => calculatePictures(),
    [columnWidth, maxSpan],
  );
  ...
  return (
    <Layout>
      ...
      <GalleryGrid windowWidth={windowWidth}>{pictures}</GalleryGrid>
